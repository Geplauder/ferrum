...
use anyhow::Context;
use ferrum_db::servers::queries::remove_user_from_server;
...
pub async fn leave(
    ...
    remove_user_from_server(&mut transaction, auth.claims.id, *server_id)
        .await
        .map_err(|_| LeaveError::NotOnServerError)?;
